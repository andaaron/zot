...
func TestSearchRoutes(t *testing.T) {
	Convey("Upload image for test", t, func(c C) {
		...
		Convey("GlobalSearch with authz enabled", func(c C) {
			...
			resp, err := resty.R().SetBasicAuth(user1, password1).Get(baseURL + constants.V2ExtSearchPrefix +
			...
			resp, err = resty.R().Get(baseURL + constants.V2ExtSearchPrefix + "?query=" + url.QueryEscape(query))
			So(err, ShouldBeNil)
			So(resp, ShouldNotBeNil)
			So(resp.StatusCode(), ShouldEqual, http.StatusUnauthorized)
...
func TestDistSpecExtensions(t *testing.T) {
	Convey("start zot server with search extension", t, func(c C) {
		...
		So(extensionList.Extensions[0].Endpoints[0], ShouldEqual, constants.V2ExtSearchPrefix)
	})
