...
//
//nolint:unparam // Some callers may not use all return values
func setupTestCerts(t *testing.T) (
	string, string, string, string, string, string, []byte,
) {
	t.Helper()
	tempDir := t.TempDir()
	...
	return caCertPath, caKeyPath, serverCertPath, serverKeyPath, clientCertPath, clientKeyPath, caCertPEM
}
...
func TestScaleOutRequestProxy(t *testing.T) {
	...
	Convey("Given a zot scale out cluster in https mode with auth enabled", t, func() {
		...
		caCertPath, _, serverCertPath, serverKeyPath, _, _, caCertPEM := setupTestCerts(t)
	...
	Convey("Given a zot scale out cluster in with 2 members and an incorrect RootCACert", t, func() {
		...
		_, _, serverCertPath, serverKeyPath, _, _, caCertPEM := setupTestCerts(t)
...
func TestMutualTLSAuthWithUserPermissions(t *testing.T) {
	Convey("Make a new controller", t, func() {
		caCertPath, _, serverCertPath, serverKeyPath, clientCertPath, clientKeyPath, caCertPEM := setupTestCerts(t)
...
func TestAuthnErrors(t *testing.T) {
	...
	Convey("ldap CA certs is empty", t, func() {
		...
		conf.HTTP.Port = port
		// Generate certificates dynamically for the test
		caCertPath, _, _, _, _, _, _ := setupTestCerts(t)
...
func TestTSLFailedReadingOfCACert(t *testing.T) {
	Convey("no permissions", t, func() {
		caCertPath, _, serverCertPath, serverKeyPath, _, _, _ := setupTestCerts(t)
		port := test.GetFreePort()
		conf := config.New()
		conf.HTTP.Port = port
		conf.HTTP.TLS = &config.TLSConfig{
	...
	Convey("empty CACert", t, func() {
		...
		_, _, serverCertPath, serverKeyPath, _, _, _ := setupTestCerts(t)
		port := test.GetFreePort()
		conf := config.New()
		conf.HTTP.Port = port
		conf.HTTP.TLS = &config.TLSConfig{
