...
const (
	...
	tag = "0.0.1"     //nolint:gochecknoglobals
)
...
func newHTTPSProxyServer(target string) (*httpsProxyServer, error) {
	...
	// Note: This requires root privileges or CAP_NET_BIND_SERVICE capability
...
var httpsProxy *httpsProxyServer
...
func setupHostsEntries() error {
	...
	for _, entry := range entries {
		...
		cmd := exec.Command("grep", "-q", strings.Fields(entry)[1], "/etc/hosts")
		if cmd.Run() == nil {
			// Entry already exists, skip
			continue
		}

		// Add entry using tee (requires privileges)
		cmd = exec.Command("sh", "-c", fmt.Sprintf("echo '%s' >> /etc/hosts", entry))
...
func teardownHostsEntries() {
	...
	for _, domain := range domains {
		// Remove entry using sed (requires privileges)
		cmd := exec.Command("sed", "-i", fmt.Sprintf("/%s/d", strings.ReplaceAll(domain, ".", "\\.")), "/etc/hosts")
...
func TestMain(m *testing.M) {
	...
	if os.Getenv("GCSMOCK_ENDPOINT") != "" {
		...
		if err != nil {
			// If we can't bind to port 443, log a warning but don't fail
			// The tests might still work if /etc/hosts is set up correctly
			fmt.Printf("Warning: Could not start HTTPS proxy on port 443: %v\n", err)
			fmt.Printf("This may require root privileges or CAP_NET_BIND_SERVICE capability\n")
			httpsProxy = nil
		} else {
			httpsProxy.Start()
