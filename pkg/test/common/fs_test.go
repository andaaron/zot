...
func TestCopyFiles(t *testing.T) {
	...
	Convey("destDir is a file", t, func() {
		...
		err := testc.CopyFiles("../../test/data", dir)
		So(err, ShouldBeNil)
...
func TestCopyFile(t *testing.T) {
	Convey("destFilePath does not exist", t, func() {
		err := testc.CopyFile("/path/to/srcFile", "~/path/to/some/unexisting/destDir/file")
		So(err, ShouldNotBeNil)
	})

	Convey("sourceFile does not exist", t, func() {
		err := testc.CopyFile("/path/to/some/unexisting/file", path.Join(t.TempDir(), "destFile.txt"))
		So(err, ShouldNotBeNil)
	})
}
