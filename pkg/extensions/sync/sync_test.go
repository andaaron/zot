...
func makeUpstreamServer(
	...
	if secure {
		tempDir := t.TempDir()
		_, _, _, _, _, _, caCertPEM := setupTestCertsForSync(t, tempDir)
...
func TestTLS(t *testing.T) {
	Convey("Verify sync TLS feature", t, func() {
		...
		caCertPath, _, _, _, clientCertPath, clientKeyPath, caCertPEM := setupTestCertsForSync(t, sharedCertDir)
...
func TestBearerAuth(t *testing.T) {
	...
	for _, testCase := range testCases {
		Convey("Verify periodically sync bearer auth with "+testCase.name, t, func() {
			...
			tempDir := t.TempDir()
			_, _, serverCertPath, serverKeyPath, _, _, _ := setupTestCertsForSync(t, tempDir)
...
func TestInvalidCerts(t *testing.T) {
	Convey("Verify sync with bad certs", t, func() {
		...
		caCertPath, _, _, _, _, _, _ := setupTestCertsForSync(t, clientCertDir)
