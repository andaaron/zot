...
func TestGlobalSearch(t *testing.T) {
	...
	Convey("test nested indexes", t, func() {
		...
		indexMultiArchMiddle1Digest, _, err := storeCtlr.GetDefaultImageStore().PutImageManifest(repoName, "multiArchMiddle1", ispec.MediaTypeImageIndex,
			indexMultiArchMiddle1Blob)
		So(err, ShouldBeNil)
		...
		indexMultiArchMiddle2Digest, _, err := storeCtlr.GetDefaultImageStore().PutImageManifest(repoName, "multiArchMiddle2", ispec.MediaTypeImageIndex,
			indexMultiArchMiddle2Blob)
		So(err, ShouldBeNil)
		...
		for _, image := range results.Images {
			...
			if image.Tag == "multiArchMiddle1" {
				So(len(image.Manifests), ShouldEqual, 4)

			} else if image.Tag == "multiArchMiddle2" {
				So(len(image.Manifests), ShouldEqual, 2)

			} else if image.Tag == "multiArchTop" {
