...
func (gen *DBUpdateTaskGenerator) Next() (scheduler.Task, error) {
	...
	if gen.status == pending && time.Since(gen.lastTaskTime) >= gen.waitTime {
		newTask = newDBUpdadeTask(gen.interval, gen.cveInfo, gen, gen.log)
		gen.status = running
	}
	gen.lock.Unlock()
