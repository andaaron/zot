...
var (
	errBadTest    = errors.New("There's something wrong with the test")
	errFailedScan = errors.New("The scan has failed intentionally")
)

func TestScanGeneratorWithMockedData(t *testing.T) {
	Convey("Test CVE scanning task scheduler with diverse mocked data", t, func() {
		...
		So(err, ShouldBeNil)
		err = metaDB.SetRepoReference("repo4", "invalid-config", image41.ManifestDescriptor.Digest, ispec.MediaTypeImageManifest)
		So(err, ShouldBeNil)
		...
		// This is normaly done in MetaDB, but we want to verify
		...
		scanner := mocks.CveScannerMock{
			ScanImageFn: func(image string) (map[string]cvemodel.CVE, error) {
				...
				if repo == repo1 &&
					zcommon.Contains([]string{image12Digest, image21Digest}, ref) {
		...
		// Start the generator
		generator := cveinfo.NewScanTaskGenerator(logger, metaDB, scanner)
...
func TestScanGeneratorWithRealData(t *testing.T) {
	Convey("Test CVE scanning task scheduler real data", t, func() {
		...
		So(cveInfo.Scanner.IsResultCached(image.DigestStr()), ShouldBeFalse)

		sch := scheduler.NewScheduler(cfg, logger)

		generator := cveinfo.NewScanTaskGenerator(logger, metaDB, cveInfo.Scanner)
		...
		So(cveInfo.Scanner.IsResultCached(image.DigestStr()), ShouldBeTrue)

		cveMap, err := cveInfo.Scanner.ScanImage("zot-test:0.0.1")
		So(err, ShouldBeNil)
