...
The CVE (Common Vulnerabilities and Exposures) scanning extension provides vulnerability detection for container images stored in zot. It uses [Trivy](https://github.com/aquasecurity/trivy), a comprehensive security scanner, to identify vulnerabilities in container images.
...
3. **Automatic Background Scanning**: Images are automatically scanned in the background by a scheduler task that runs every 15 minutes (hardcoded, not configurable). This task starts as soon as zot starts and the Trivy database is downloaded. The scheduler checks for unscanned images and processes them in the background. **Note**: Scanning does not occur on image push; it happens asynchronously via the scheduler.
...
**Note**: Images with unsupported layer media types cannot be scanned and will return an error.

## How to Enable CVE Scanning
...
2. **Storage Access**: The zot process needs write access to the storage root directory to create the `_trivy` cache directories.

3. **Temporary Directory Space**: The Trivy library requires sufficient space on the temporary partition (`/tmp` by default). This space is needed for:
   ...
   Ensure your system has adequate `/tmp` space, especially if scanning images with large layers.
...
#### Full Configuration with Custom Trivy Databases
...
**Important Notes**:
- The `updateInterval` is **optional**. If not specified, it defaults to 2 hours. If a value less than 2 hours is specified, it will be automatically adjusted to 2 hours (as enforced by the code in `pkg/cli/server/root.go`).
...
The zot CLI provides several commands for querying CVE information:
...
```bash
zot cve list <repo>:<tag>
# or using digest
zot cve list <repo>@<digest>
```

**Options**:
- `--cve-id`: Filter results to a specific CVE ID
- `--sort-by`: Sort results (options: `severity`, `alpha-asc`, `alpha-dsc`)

**Examples**:
```bash
# Basic usage - list all CVEs in an image by tag
zot cve list alpine:latest

# List CVEs using image digest
zot cve list myapp@sha256:abc123def456...

# Filter to show only a specific CVE
zot cve list alpine:latest --cve-id CVE-2021-44228

# Sort results by severity (default)
zot cve list alpine:latest --sort-by severity

# Sort results by CVE ID alphabetically (ascending)
zot cve list alpine:latest --sort-by alpha-asc

# Sort results by CVE ID alphabetically (descending)
zot cve list alpine:latest --sort-by alpha-dsc

# Combine options: filter and sort
zot cve list myapp:latest --cve-id CVE-2021 --sort-by severity
```
...
```bash
zot cve affected <CVE-ID>
```

**Options**:
- `--repo`: Filter results to a specific repository
- `--sort-by`: Sort results (options: `update-time`, `alpha-asc`, `alpha-dsc`)

**Examples**:
```bash
# List all images affected by a CVE across all repositories
zot cve affected CVE-2021-44228

# Filter to show only images in a specific repository
zot cve affected CVE-2021-44228 --repo myapp

# Sort results by update time (most recent first)
zot cve affected CVE-2021-44228 --sort-by update-time

# Sort results alphabetically (ascending)
zot cve affected CVE-2021-44228 --sort-by alpha-asc

# Combine options: filter by repo and sort
zot cve affected CVE-2021-44228 --repo myapp --sort-by update-time
```
...
zot cve fixed myapp CVE-2021-44228

# Sort results by update time (most recent first)
zot cve fixed myapp CVE-2021-44228 --sort-by update-time

# Sort results alphabetically (ascending)
zot cve fixed myapp CVE-2021-44228 --sort-by alpha-asc

# Sort results alphabetically (descending)
zot cve fixed myapp CVE-2021-44228 --sort-by alpha-dsc
```
...
```bash
zot cve diff <image1> <image2>
# or with platform specifications
zot cve diff <image1> [platform1] <image2> [platform2]
```

**Examples**:
```bash
# Compare CVEs between two images by tag
zot cve diff myapp:v1.0 myapp:v2.0

# Compare CVEs between two images using digest
zot cve diff myapp@sha256:abc123... myapp@sha256:def456...

# Compare CVEs between images with platform specification for first image
zot cve diff myapp:v1.0 linux/amd64 myapp:v2.0

# Compare CVEs between images with platform specifications for both images
zot cve diff myapp:v1.0 linux/amd64 myapp:v2.0 linux/arm64

# Compare CVEs between different repositories
zot cve diff prod/app:v1.0 staging/app:v2.0
```
...
If you see errors about the CVE database not being found:

1. **Check Storage Permissions**: Ensure zot has write access to the storage directory
2. **Check Temporary Directory**: Ensure `/tmp` has sufficient space and write permissions, as databases are initially downloaded there
3. **Wait for Initial Download**: The database download happens asynchronously; wait a few minutes after enabling CVE scanning
4. **Check Logs**: Look for database download errors in the zot logs
5. **Manual Verification**: Check that `{storageRoot}/_trivy/db/trivy.db` exists

### Scan Failures

If image scans are failing:

1. **Check Image Format**: Ensure the image uses supported layer media types
2. **Check Logs**: Review zot logs for specific error messages
3. **Database Status**: Verify the Trivy database is up to date
4. **Image Accessibility**: Ensure the image exists and is accessible

## Security Considerations

1. **Database Source**: Only use trusted sources for Trivy databases. The default repositories (`ghcr.io/aquasecurity/*`) are maintained by Aqua Security
2. **Network Access**: zot needs network access to download databases from container registries
3. **Storage Security**: The `_trivy` directories contain vulnerability data and should be protected accordingly
4. **Scan Results**: CVE information may reveal details about installed packages; consider access controls

## Limitations

1. **Offline Scanning Only**: zot uses Trivy in offline mode; it doesn't perform real-time vulnerability lookups
2. **Database Updates**: Vulnerability information is only as current as the last database update
...
- [Search Extension Documentation](../README.md)
- [Trivy Documentation](https://github.com/aquasecurity/trivy)
- [GraphQL API Documentation](../../search/search.md)
